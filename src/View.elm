...
drawLineyMaze maze =
    ...
    let
        ...
        cellBorders =
            let
                drawCellBoundaries : Position -> Cell -> List (S.Svg a)
                drawCellBoundaries position cell =
                    drawCell scale position cell
            in
            Grid.indexedMap drawCellBoundaries maze
                |> Grid.toList
                |> List.concat

        cellColors =
            let
                drawBackroundColor : Position -> Cell -> List (S.Svg a)
                drawBackroundColor position _ =
                    let
                        color =
                    ...
                    Maybe.map (\c -> [ colorCell scale position c ]) color
                        |> Maybe.withDefault []
            in
            Grid.indexedMap drawBackroundColor maze
                |> Grid.toList
                |> List.concat
